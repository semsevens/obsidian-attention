...
**从渲染出的 `<img>` 反查源码用的是 `src` 匹配，不是数第几张图** ——
实时预览只渲染视口附近的 widget，按位置数恰好会在笔记长到值得数的时候出错。
同一张图用了两次时，才在它自己的重复项之间按顺序区分。
...
**左键不拦截**：点图片是 Obsidian 自己的放大手势，抢过来弹气泡会让被标记的图片
表现得和 vault 里其它图片不一样。评论改成 `title` 悬停显示，完整菜单走右键。
